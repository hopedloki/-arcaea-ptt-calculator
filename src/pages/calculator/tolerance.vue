...
/**
 * 容错计算器
 * 提供评级容错、分数容错、PTT容错三种计算模式
 * 每种模式支持两套计算方式：基于当前判定（输入Pure/Far/Lost数量）和基于理论值（从理论满分反推）
 * 监听 songSelected 全局事件接收歌曲选择，自动从本地存储加载最近使用的歌曲
 */
...
// 页面挂载 — 加载最近使用的歌曲，监听 songSelected 事件
onMounted(() => {
  // 从存储中获取最近使用的歌曲
  const recentSong = uni.getStorageSync(STORAGE_KEYS.RECENT_SONG)
  if (recentSong) {
...
// 页面加载 — 检查是否有从歌曲详情/列表页传来的选曲数据，自动填入物量
...
// 评级选择变化
const onRatingChange = (e: { detail: { value: number } }) => {
  ratingIndex.value = e.detail.value
}

// 执行容错计算 — 根据当前模式和计算方式分发到对应计算函数
const calculate = () => {
