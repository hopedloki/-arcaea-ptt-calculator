...
/**
 * 成绩↔PTT 双向转换计算器
 * 支持两种模式：成绩→PTT（输入分数计算对应PTT）和 PTT→成绩（输入目标PTT反推所需最低分数）
 * 定数可通过歌曲选择或手动输入两种方式提供，计算结果可直接保存到 B30 记录
 * 跨页面通信统一走 storage 单通道：由 onPageShow 读取 SELECTED_SONG_CALC/RECENT_SONG
 */
...
// 转换计算结果
const result = ref<PttCalculationResult | null>(null)
...
// 是否满足计算条件 — 需要有有效定数和合法输入值
const canCalculate = computed(() => {
  // 检查定数是否可用
  let constant = 0
  if (inputMode.value === 'song') {
    constant = selectedSong.value?.constant ?? 0
  } else {
...
// 是否允许保存到 B30 — 需要有有效的歌曲名称或手动输入的定数
...
// 切换转换模式（成绩→PTT 或 PTT→成绩），清空已有结果和输入
...
// 跳转到歌曲列表页面（携带 from=calculator 参数）
const goToSongList = () => {
  uni.navigateTo({
...
// 执行转换计算 — 根据当前模式和输入模式计算 PTT 或所需成绩
const calculate = () => {
  // 获取定数
  let constant = 0
  let songName = ''
...
const saveToBest30 = () => {
  ...
  try {
    ...
    best30Records.sort((a: Best30Record, b: Best30Record) => b.ptt - a.ptt)
    ...
    setTimeout(() => {
      showConfirm('提示', '已成功保存到B30，是否跳转到B30管理页面？').then((confirmed) => {
        if (confirmed) {
          uni.navigateTo({
