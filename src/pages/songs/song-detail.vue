<template>
	<view class="container">
		<view class="card song-detail-card" v-if="songData">
			<!-- 歌曲基本信息 -->
			...
			<view class="difficulties-section">
				...
				<view class="difficulties-list">
					<view class="difficulty-detail-card" v-for="(chart, key) in getAvailableCharts()" :key="key"
...
<script setup lang="ts">
	/**
	 * 歌曲详情页
	 * 展示单首歌曲的基本信息（名称/曲师/曲包/BPM）及各难度谱面详情（定数/物量/谱师/等级）
	 * 同时为每个可用谱面展示 EX+、EX、AA 三个评级的简略容错（最大可Far数）
	 * 数据来源为本地存储中已缓存的歌曲数据库
	 */
	import { ref } from 'vue'
	...
	import type { SimpleSongData, ChartData } from '@/types'
	...
	// 加载歌曲详情
	const loadSongDetail = (songId : string) => {
		try {
			// 从本地存储加载歌曲数据
	...
	// 获取可用谱面 — 兼容 charts 数组格式和 SimpleSongData 扁平字段两种数据结构
	const getAvailableCharts = () => {
		const result: Record<string, Partial<ChartData>> = {}
		if (!songData.value) return result
		const difficulties = ['pst', 'prs', 'ftr', 'byd', 'etr']

		// 从本地存储加载的歌曲数据是SimpleSongData格式（有pst/prs/ftr等字段）
		// 需要检查两种格式

		// 兼容完整 SongData 格式（带 charts 数组），SimpleSongData 不含该字段，使用类型断言访问
		const songAny = songData.value as SimpleSongData & { charts?: ChartData[] }
		if (songAny.charts && Array.isArray(songAny.charts)) {
			// 如果是完整的SongData格式，有charts数组
			songAny.charts.forEach((chart: ChartData) => {
