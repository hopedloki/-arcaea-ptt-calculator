...
export function exportAllData(includeSongs: boolean = true): AppData {
  try {
    ...
    // 导出B30记录
    const best30Records = uni.getStorageSync(STORAGE_KEYS.BEST30_RECORDS) || []
    if (best30Records.length > 0) {
      data.best30Records = best30Records
    }

    // 导出最近记录
    const recentRecords = uni.getStorageSync(STORAGE_KEYS.RECENT_SCORES) || []
    if (recentRecords.length > 0) {
      data.recentRecords = recentRecords
    }

    // 导出PTT数据
    const pttData = uni.getStorageSync(STORAGE_KEYS.PTT_DATA)
    if (pttData) {
...
export function importData(
  ...
  try {
    ...
    if (data.best30Records && Array.isArray(data.best30Records) && overwriteBest30) {
      uni.setStorageSync(STORAGE_KEYS.BEST30_RECORDS, data.best30Records)
    }

    // 导入最近记录
    if (data.recentRecords && Array.isArray(data.recentRecords) && overwriteRecent) {
      uni.setStorageSync(STORAGE_KEYS.RECENT_SCORES, data.recentRecords)
    }

    // 导入PTT数据
    if (data.pttData && overwritePTTData) {
      uni.setStorageSync(STORAGE_KEYS.PTT_DATA, data.pttData)
    }

    // 导入歌曲数据
    if (data.songsData && Array.isArray(data.songsData) && overwriteSongs) {
      uni.setStorageSync(STORAGE_KEYS.SONGS_DATA, data.songsData)
    }
...
export function clearAllData(): void {
  try {
    uni.removeStorageSync(STORAGE_KEYS.BEST30_RECORDS)
    uni.removeStorageSync(STORAGE_KEYS.RECENT_SCORES)
    uni.removeStorageSync(STORAGE_KEYS.PTT_DATA)
    // 注意：不清空歌曲数据，除非用户明确要求
  } catch (e) {
    // #ifdef dev
